...
// export const API_BASE_URL = "https://3trading.realstateinvestbackend.xyz/";
// export const API_BASE_URL2 = "https://3trading.realstateinvestbackend.xyz";
...
export const checkLuckySpinValidation = async (userId) => {
  try {
    const res = await fetch(
      `${API_BASE_URL}api/users/user-luckySpin-validationcheck`,
      {
        method: "POST",
...
export const createLuckySpin = async (userId, amount, data) => {
  try {
    const res = await fetch(
      `${API_BASE_URL}api/users/user-luckySpin-dataCreate`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
export const handleClaimRecordDB = async (
  userId,
  purchaseId,
  cycleIndex,
  claimAmount,
